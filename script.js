...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleSfxVolumeChange() {
         if (clickSound && sfxVolumeInput && sfxVolumeValueSpan) {
            ...
            if (!isNaN(volume)) {
                 ...
                 if (volume > 0 && volume > lastValue + 0.01) {
                     playClickSound();
                 }
                 sfxVolumeInput.dataset.lastValue = sfxVolumeInput.value;
            }
    ...
    async function generatePack() {
        ...
        const additionalFilesMap = new Map();
        ...
        selectedPresetIds.forEach(presetId => {
         ...
         const objectiveCommands = [
             ...
             '# Ensure objectives are added only if they don\'t exist (requires a player online when run).',
             '',
    ...
    function processNBTFile(file) {
        ...
        readFileContent(file).then(content => {
            ...
            file_name = `NiftyBuildingTool_${cleanNbtNameForFilename}.txt`;
