...
function openTab(tabId) {
 ...
 document.addEventListener('DOMContentLoaded', function() {
     openTab('qrTool'); // Opens the QR code tab by default
      ...
      // --- Add listener for range slider value update ---
      // This part is crucial for making the Webkit range track fill correctly
      const thresholdInput = document.getElementById('threshold');
      if (thresholdInput) {
          // Also update the --threshold-progress variable when the slider is moved
          thresholdInput.addEventListener('input', function() {
              const thresholdValueSpan = document.getElementById('thresholdValue');
              if (thresholdValueSpan) {
                 thresholdValueSpan.textContent = this.value; // Update display
              }
              // Update the CSS variable for Webkit track fill
              this.style.setProperty('--threshold-progress', `${(this.value / 255) * 100}%`);
          });
...
// Moved threshold input listeners into the DOMContentLoaded check
